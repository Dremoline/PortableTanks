...
public class PortableTankBlockEntity extends BaseBlockEntity implements IFluidHandler, TickableBlockEntity {
    ...
    public PortableTankBlockEntity(PortableTankType type) {
    ...
    public boolean interactWithItemFluidHandler(IFluidHandlerItem fluidHandler, PlayerEntity player) {
        ...
        if (tankFluid.isEmpty()) {
            if (!this.fluidStack.isEmpty() && fluidHandler.isFluidValid(0, this.fluidStack)) {
                ...
                if (amount > 0) {
                    SoundEvent soundEvent = this.fluidStack.getFluid().getAttributes().getFillSound();
                    if (soundEvent != null) {
